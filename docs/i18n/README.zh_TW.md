...
![一支隊伍聚集在 Greywillow 的藥劑師店外](../../docs/screenshots/party-questing.jpg)
...
Brother Cassian 的故事線延續到 *The Restless Dead* 之後：**Whispers Below**（在殘破禮拜堂找出 Bonecaller 的符印）→**The Binding Rite**（從 kobold 礦坑採集 Blessed Tallow、從不得安息的亡者身上採集 Ghostly Essence）→**Into the Hollow**（*建議玩家數：5*）——在禮拜堂下方地穴的最底層擊殺 Sarghul the Bonecaller。

- Fallen Chapel 的地穴門會把你的**隊伍傳送進專屬的私人副本拷貝**（6 個位置；副本在淨空 5 分鐘後重置）。
- 內部：火炬照明的廳堂、成對出現的**菁英**雜怪群（原版菁英縮放：約 2.3× 生命、約 1.5× 傷害、雙倍經驗值）、副王 Sexton Marrow，以及 Sarghul——一個 10 級菁英王，每 10 秒施放一次 **Shadow Pulse** 範圍攻擊。副本雜怪在副本重置前不會重生。
...
陰謀並未隨 Sarghul 而終結。**The Sunken Bastion**（5 人、約 13 級、Sablefen 東南）關著 Ysra the Fogcaller——他會在生命 60% 與 30% 時召喚一波波 Drowned Thralls。最終章是 **Wyrmbarrow Sanctum**（5 人、20 級、Thornfell 下方）：三間滿是菁英 boneguard 與 drakonid 的廳室、Durgath the Bound（生命低於 30% 時狂暴）、Grand Necromancer Nethanys（更多增援波次），以及 **Vorruth the Wyrmbarrow**——史詩武器在此掉落，而前置任務鏈可以單人完成，因此沒有人會被擋在故事之外。
...
為你的角色命名、從九個職業中任選其一，你就身處 **Greywillow Vale**（1-7 級）：一座被六個據點環繞的市集小鎮——北邊的狼群路徑、東邊的野豬草原、西邊的 the Tanglemoor、西北邊的 Mirror Lake、西南邊的 kobold 銅礦坑、東北邊有不得安息亡者的殘破禮拜堂，以及東南邊 Brakka 的盜賊營地。北方道路穿越一處山隘攀升進入 **Sablefen Marsh**（6-13，據點：Reedford），再往上通往 **Thornfell Heights**（13-20，據點：Wardenwatch）——三個區域、約 60 個任務，以及一條故事線：Bonecaller 的陰謀，從 Greywillow 外圍最初那些不得安息的枯骨，一直到山峰下方的 **Vorruth the Wyrmbarrow**。每個據點都有商人（包括販售貨真價實白色裝備的武器匠與防具匠）、一處墓地、各自的音樂，以及一張區域地圖。
...
- 怪物 AI：遊蕩、依等級差距的近距離仇恨、社交拉怪（murloc 會從更遠處拉怪——帶上朋友吧）、追擊、繫繩—脫戰—重置、屍體拾取、重生；還有一個長計時的稀有生怪（Old Grimfang）
