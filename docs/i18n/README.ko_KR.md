...
![Greywillow의 약초상 앞에 모인 파티](../../docs/screenshots/party-questing.jpg)
...
Brother Cassian의 이야기 줄기는 *The Restless Dead* 이후로 계속됩니다: **Whispers Below**(폐허가 된 예배당에서 Bonecaller의 인장 찾기) → **The Binding Rite**(코볼트 채굴장에서 Blessed Tallow를, 안식 없는 망자에게서 Ghostly Essence를 모으기) → **Into the Hollow**(*권장 인원: 5명*) — 예배당 아래 지하 묘지 맨 밑에서 Sarghul the Bonecaller를 처치하세요.

- Fallen Chapel의 지하 묘지 문은 **당신의 파티를 전용 비공개 인스턴스 사본**으로 순간이동시킵니다(슬롯 6개; 인스턴스는 비어 있은 지 5분 후 리셋됩니다).
- 내부: 횃불이 밝혀진 복도, 짝지어진 **정예** 잡몹 무리(바닐라 정예 스케일링: 체력 ~2.3배, 피해 ~1.5배, XP 2배), 중간 보스 Sexton Marrow, 그리고 Sarghul — 10초마다 **Shadow Pulse** 광역기를 쓰는 레벨 10 정예 보스. 던전 잡몹은 인스턴스가 리셋될 때까지 다시 출현하지 않습니다.
...
음모는 Sarghul으로 끝나지 않습니다. **The Sunken Bastion**(5인, ~레벨 13, Sablefen 남동부)에는 Ysra the Fogcaller가 있으며 — 그는 체력 60%와 30%에서 Drowned Thrall 무리를 소환합니다. 대미는 **Wyrmbarrow Sanctum**(5인, 레벨 20, Thornfell 아래)입니다: 정예 boneguard와 drakonid가 있는 세 개의 방, Durgath the Bound(체력 30% 미만에서 격노), Grand Necromancer Nethanys(추가 소환 무리), 그리고 **Vorruth the Wyrmbarrow** — 이곳에서 에픽 무기가 드롭되며, 이어지는 퀘스트 사슬은 솔로로 진행할 수 있어 아무도 이야기에서 소외되지 않습니다.
...
캐릭터 이름을 짓고, 아홉 직업 중 아무거나 고르면, **Greywillow Vale**(레벨 1-7)에 들어와 있습니다: 여섯 개의 거점으로 둘러싸인 시장 마을 — 북쪽의 늑대 사냥터, 동쪽의 멧돼지 초원, 서쪽의 the Tanglemoor, 북서쪽의 Mirror Lake, 남서쪽의 코볼트 구리 채굴장, 북동쪽의 안식 없는 망자가 있는 폐허가 된 예배당, 그리고 남동쪽의 Brakka의 도적 야영지. 북쪽 길은 산길을 따라 올라 **Sablefen Marsh**(6-13, 거점: Reedford)로 이어지고, 다시 위로 **Thornfell Heights**(13-20, 거점: Wardenwatch)까지 올라갑니다 — 세 개의 구역, ~60개의 퀘스트, 그리고 하나의 이야기 줄기: Greywillow 외곽의 첫 안식 없는 뼈에서부터 봉우리 아래의 **Vorruth the Wyrmbarrow**에 이르는 Bonecaller 음모. 각 거점에는 상인(정직한 흰색 장비를 파는 무기 대장장이와 방어구 대장장이 포함), 묘지, 고유한 음악, 그리고 구역 지도가 있습니다.
...
- 몹 AI: 배회, 레벨 차이에 따른 근접 어그로, 사회적 풀(murloc은 더 멀리서 끌려옵니다 — 친구를 데려오세요), 추격, 끈-회피-리셋, 시체 약탈, 재출현; 긴 타이머로 도는 희귀 출현 몹(Old Grimfang)
