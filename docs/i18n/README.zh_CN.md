...
# Legends of Aldermoor —— 一款经典风格 MMO

[加入社区 Discord](https://discord.gg/GjhnUsBtw)

![Legends of Aldermoor 标题画面](../../docs/screenshots/title-screen.jpg)
...
![一支小队聚集在 Greywillow 的药剂师店外](../../docs/screenshots/party-questing.jpg)
...
![Legends of Aldermoor 社区](../../woc_community.png)
...
Brother Cassian 的剧情线在 *The Restless Dead* 之后继续：**Whispers Below**
（在废弃教堂找到 Bonecaller 的印记）→ **The Binding Rite**（从 kobold 挖掘点收集
Blessed Tallow，从不安的亡者身上收集 Ghostly Essence）→ **Into the Hollow**
（*建议玩家数：5*）—— 在教堂下方地穴的最深处击杀 Sarghul the Bonecaller。

- Fallen Chapel 处的地穴门会把你的**小队传送进它专属的私有副本拷贝**（6 个名额；
  空置 5 分钟后副本重置）。
- 副本内部：火把照明的厅堂、成对的**精英**杂兵群（初版精英缩放：~2.3× 生命、
  ~1.5× 伤害、双倍经验）、小 Boss Sexton Marrow，以及 Sarghul —— 一个 10 级精英
...
## The Sunken Bastion 与 Wyrmbarrow Sanctum

阴谋并未随着 Sarghul 而终结。**The Sunken Bastion**（5 人，~13 级，Sablefen 东南部）
盘踞着 Ysra the Fogcaller —— 他会在生命值 60% 和 30% 时召唤成波的 Drowned Thralls。
终章是 **Wyrmbarrow Sanctum**（5 人，20 级，Thornfell 下方）：三间满是精英 boneguard
和 drakonid 的密室，Durgath the Bound（低于 30% 时进入狂暴）、Grand Necromancer
Nethanys（更多增援波次），以及 **Vorruth the Wyrmbarrow** —— 史诗武器在此掉落，而且
...
为你的角色取名，从九个职业中任选其一，你就来到了 **Greywillow Vale**（1-7 级）：
一座被六个据点环绕的集市镇 —— 北面是狼群活动区，东面是野猪草甸，西面是 the
Tanglemoor，西北是 Mirror Lake，西南是一处 kobold 铜矿挖掘点，东北是一座聚集着不安
亡者的废弃教堂，东南则是 Brakka 的强盗营地。向北的道路穿过一处山口爬升进入
**Sablefen Marsh**（6-13，据点：Reedford），再往上抵达 **Thornfell Heights**
（13-20，据点：Wardenwatch）—— 三个区域、约 60 个任务，以及一条贯穿始终的剧情线：
Bonecaller 的阴谋，从 Greywillow 外第一具不安的尸骨，一直到群峰之下的 **Vorruth the
Wyrmbarrow**。每个据点都有商人（包括出售货真价实白色装备的武器匠与护甲匠）、一座
墓地、各自的音乐，以及一张区域地图。
