...
// `professionsState` is now real for the gathering professions (#1119);
// crafting/secondary professions still contribute nothing until
// #1120/#1125/#1126/#1140 land. `nodeHarvestableByMe` (#1121) is per-VIEWER,
// never global: whether the given gather node (see
// src/sim/content/gather_nodes.ts, #1120) is harvestable right now BY THE
// LOCAL VIEWER specifically. Two players asking about the same node id can
// get different answers, because each player's respawn timer for a node is
// independent (see src/sim/professions/gathering.ts).
