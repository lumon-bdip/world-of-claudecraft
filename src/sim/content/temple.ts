...
import type {
  CampDef, DungeonDef, DungeonSpawn, GroundObjectDef, ItemDef, MobTemplate,
  NpcDef, PlayerClass, QuestDef, ZonePropsDef,
} from '../types';
...
export const TEMPLE_NPCS: Record<string, NpcDef> = {
  tidewatcher_ondrel: {
    id: 'tidewatcher_ondrel', name: 'Ondrel Vane', title: 'Tidewatcher',
    pos: { x: -66, z: 786 }, facing: -2.4, color: 0x7fa6c9,
    questIds: ['q_glimmermere_light', 'q_tarn_waders', 'q_drowned_choir', 'q_palecoil', 'q_silence_the_choir', 'q_drowned_moon'],
    greeting: 'The mere drinks the moonlight, $C, and gives back the drowned. I have watched that gate for thirty nights — and tonight it is open.',
  },
};
...
export const TEMPLE_QUESTS: Record<string, QuestDef> = {
  q_glimmermere_light: {
    id: 'q_glimmermere_light', name: 'Light on the Water',
    ...
    completionText: 'A waking-prayer... to something they called the Drowned Moon. And below it, in a steadier hand: "It only sleeps." The water has been listening a long time, $N.',
    objectives: [{ type: 'collect', itemId: 'moongate_rubbing', count: 1, label: 'Warding Rubbing taken' }],
    xpReward: 3200, copperReward: 1500, itemRewards: {},
    minLevel: 15,
  },
  q_tarn_waders: {
  ...
  q_drowned_choir: {
    id: 'q_drowned_choir', name: 'The Drowned Choir',
    giverNpcId: 'tidewatcher_ondrel', turnInNpcId: 'tidewatcher_ondrel',
    ...
    completionText: 'Pearls, knuckle-bones, a child\'s carved fish... grave-gifts, $N. They are not raising the dead. They are dressing them, the way you dress a body for burial. The temple is a tomb that refuses to close.',
    objectives: [
    ...
    xpReward: 3800, copperReward: 1800,
    itemRewards: { warrior: 'tidewatchers_wraps', mage: 'tidewatchers_wraps', rogue: 'tidewatchers_wraps' },
    requiresQuest: 'q_tarn_waders',
  },
  q_palecoil: {
    id: 'q_palecoil', name: 'Sethrael the Palecoil',
    giverNpcId: 'tidewatcher_ondrel', turnInNpcId: 'tidewatcher_ondrel',
    ...
    completionText: 'Cold as the bottom of the world, and still it twitches. The shelf is clear, $N — the stair to the gate stands open. I almost wish it did not.',
    objectives: [{ type: 'collect', itemId: 'palecoil_heartscale', count: 1, label: "Sethrael's Heartscale" }],
    xpReward: 4000, copperReward: 2000,
    itemRewards: { warrior: 'moonscale_saber', mage: 'palecoil_rod', rogue: 'tideglass_dirk' },
    requiresQuest: 'q_tarn_waders', minLevel: 16,
  },
  q_silence_the_choir: {
    id: 'q_silence_the_choir', name: 'Silence the Choir',
    giverNpcId: 'tidewatcher_ondrel', turnInNpcId: 'tidewatcher_ondrel',
    ...
    completionText: 'The prayer falters... and for the first time in thirty nights, the mere is quiet. But quiet is not the same as ended. Selthe was only the choir. Something below it still listens.',
    objectives: [{ type: 'kill', targetMobId: 'choirmother_selthe', count: 1, label: 'Choirmother Selthe slain' }],
    xpReward: 4400, copperReward: 2400,
    itemRewards: { warrior: 'drownstep_sabatons', mage: 'drownstep_slippers', rogue: 'drownstep_treads' },
    requiresQuest: 'q_drowned_choir', minLevel: 16, suggestedPlayers: 5,
  },
  q_drowned_moon: {
    id: 'q_drowned_moon', name: 'The Drowned Moon',
    giverNpcId: 'tidewatcher_ondrel', turnInNpcId: 'tidewatcher_ondrel',
    ...
    completionText: 'The altar is dark, the water is still, and the moon over the tarn is only the moon. You drowned a goddess tonight, $N — and the mountain will never know how close it came. Let the wardens of the shore-rocks rest easy at last.',
    objectives: [{ type: 'kill', targetMobId: 'ysolei', count: 1, label: 'Ysolei, Avatar of the Drowned Moon, slain' }],
    xpReward: 5000, copperReward: 12000,
    itemRewards: { warrior: 'drownedmoon_maul', mage: 'drownedmoon_scepter', rogue: 'drownedmoon_kris' },
    requiresQuest: 'q_silence_the_choir', minLevel: 16, suggestedPlayers: 5,
  },
};

export const TEMPLE_QUEST_ORDER = [
  'q_glimmermere_light', 'q_tarn_waders', 'q_drowned_choir', 'q_palecoil',
...
export const TEMPLE_OBJECTS: GroundObjectDef[] = [
  {
    ...
    positions: [{ x: -74, z: 786 }, { x: -68, z: 783 }, { x: -76, z: 796 }, { x: -64, z: 794 }],
  },
];
...
export const TEMPLE_ITEMS: Record<string, ItemDef> = {
  // --- quest items ---
  moongate_rubbing: { id: 'moongate_rubbing', name: 'Warding Rubbing', kind: 'quest', sellValue: 0, questId: 'q_glimmermere_light' },
  drowned_offering: { id: 'drowned_offering', name: 'Drowned Offering', kind: 'quest', sellValue: 0, questId: 'q_drowned_choir' },
  palecoil_heartscale: { id: 'palecoil_heartscale', name: "Sethrael's Heartscale", kind: 'quest', sellValue: 0, questId: 'q_palecoil' },
  ...
  moonscale_saber: {
    id: 'moonscale_saber', name: 'Moonscale Saber', kind: 'weapon', slot: 'mainhand', quality: 'uncommon',
    weapon: { min: 17, max: 28, speed: 2.4 }, stats: { str: 5, sta: 2 }, sellValue: 700, requiredClass: WAR,
  },
  palecoil_rod: {
    id: 'palecoil_rod', name: 'Palecoil Rod', kind: 'weapon', slot: 'mainhand', quality: 'uncommon',
    weapon: { min: 18, max: 31, speed: 3.0 }, stats: { int: 6, spi: 2 }, sellValue: 700, requiredClass: MAG,
  },
  tideglass_dirk: {
    id: 'tideglass_dirk', name: 'Tideglass Dirk', kind: 'weapon', slot: 'mainhand', quality: 'uncommon',
    weapon: { min: 11, max: 18, speed: 1.7, dagger: true }, stats: { agi: 6 }, sellValue: 700, requiredClass: ROG,
  },
  ...
  drownedmoon_maul: {
    id: 'drownedmoon_maul', name: 'Drowned Moon Maul', kind: 'weapon', slot: 'mainhand', quality: 'rare',
    weapon: { min: 24, max: 38, speed: 2.6 }, stats: { str: 8, sta: 4 }, sellValue: 2200, requiredClass: WAR,
  },
  drownedmoon_scepter: {
    id: 'drownedmoon_scepter', name: 'Drowned Moon Scepter', kind: 'weapon', slot: 'mainhand', quality: 'rare',
    weapon: { min: 26, max: 42, speed: 3.0 }, stats: { int: 10, spi: 4 }, sellValue: 2200, requiredClass: MAG,
  },
  drownedmoon_kris: {
    id: 'drownedmoon_kris', name: 'Drowned Moon Kris', kind: 'weapon', slot: 'mainhand', quality: 'rare',
  ...
  // --- junk (gray) ---
  pale_pearl: { id: 'pale_pearl', name: 'Pale Pearl', kind: 'junk', quality: 'poor', sellValue: 30 },
  moonpale_scale: { id: 'moonpale_scale', name: 'Moonpale Scale', kind: 'junk', quality: 'poor', sellValue: 26 },
  briny_idol: { id: 'briny_idol', name: 'Briny Idol', kind: 'junk', quality: 'poor', sellValue: 32 },
};
