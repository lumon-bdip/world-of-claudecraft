...
| Group | Files | Needs |
|---|---|---|
| Gate / CI | `gate.mjs` (`npm run gate`): local mirror of the ci.yml pr-gate steps, sequential, stops at the first failure; release-tier (`I18N_RELEASE_TIER=1`) automatic on `release/**` branches; hard-exits if `ffmpeg`/`ffprobe` are missing from PATH (preflight pinned by `tests/sfx_gate_preflight.test.ts`). Keep its step list in sync with `.github/workflows/ci.yml`. | FFmpeg on PATH |
...
| SFX / audio | `sfx_conform.mjs` (`sfx:check` gate / `sfx:conform` = `--fix`; bundles `ffmpeg-static`/`ffprobe-static`), pure rules/manifest logic in `scripts/sfx/` (`sfx_conform_rules.mjs`, `sfx_manifest_builder.mjs`); generators `gen_sfx.mjs` (`sfx:gen`), `gen_ui_sfx.mjs` (`sfx:ui`), `gen_npc_voices.mjs`/`gen_npc_lines.mjs` (+ `voices/`), `render_music.mjs` (+ `music_render_entry.ts`); the SFX Studio `sfx_studio/` (`sfx:studio`, spawns PATH `ffmpeg`, own CLAUDE.md; tutorial: `docs/sfx-studio-tutorial.md`) | varies (ffmpeg, API keys) |
