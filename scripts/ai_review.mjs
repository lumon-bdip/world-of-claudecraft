...
//
// HARDENING (fork PRs + prompt injection): the diff the agent reviews is attacker
// controlled and its final message is posted publicly, so (a) the agent child process
// gets a minimal allowlisted environment (never GITHUB_TOKEN, never CODEX_AUTH_JSON,
// never the GITHUB_* context vars; auth is materialized into a private CODEX_HOME file
// instead), and (b) the message is scrubbed with redact_secrets.mjs before posting.
//
...
// CODEX_AUTH_JSON (already materialized into CODEX_HOME/auth.json above), and the
// GITHUB_* context vars, is withheld: the prompt embeds an attacker-controlled diff,
// so the agent must never hold credentials it could be injected into leaking.
